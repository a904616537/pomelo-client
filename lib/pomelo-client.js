...
ePomelo.setDebug = function(debug){
	DEBUG = !!debug;
}
...
pomelo.init = function (params, cb) {
	...
	var socket = this._socket = io.connect(url, {'force new connection': true, reconnect: true});
	socket.on('connect', function () {
		DEBUG && console.log('[pomeloclient.init] websocket connected!');
		if (cb) {
			cb(socket);
		}
	});
...
var processMessage = function (pomelo, msg) {
	...
	if (msg.id) {
		...
		if (typeof cb !== 'function') {
			DEBUG && console.log('[pomeloclient.processMessage] cb is not a function for request ' + msg.id);
			return;
		}
		cb(msg.body);
		return;
